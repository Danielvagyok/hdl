...
module axi_hdmi_tx_core #(
 ...
 assign hdmi_raddr_oh_s = {hdmi_raddr[510:1] & hdmi_raddr[509:0],
  ...
  always @(posedge reference_clk) begin
    if (reference_rst == 1'b1 || hdmi_fs == 1'b1) begin
      hdmi_data_sel <= 1'b0;
    end else if (hdmi_de_s == 1'b1) begin
      hdmi_data_sel <= ~hdmi_data_sel;
    end
  end      

  always @(posedge reference_clk) begin
    if (reference_rst == 1'b1) begin
      hdmi_raddr <= {2'b11, 510'd0};
    end else if (hdmi_fs == 1'b1) begin
      hdmi_raddr <= {hdmi_fs_waddr[1:0], hdmi_fs_waddr[511:2]};
